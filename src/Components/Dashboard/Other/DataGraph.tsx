import { Card, Spinner, Tab, Tabs } from "@heroui/react";
...
export default function DataGraph() {
  ...
  return (
    <Card className="rounded-xl shadow-md border border-foreground/5 p-6 hover:shadow-lg transition-shadow duration-200">
      {isLoading ? (
              ...
              <div className="mt-2 flex w-full items-center">
                <div className="-my-3 flex w-full max-w-[800px] items-center justify-center gap-x-3 overflow-x-auto py-3">
                  {metrics.map(({ key, title, icon }) => (
                    <button
                      key={key}
                      className={`flex w-full flex-col gap-2 rounded-medium p-3 transition-colors border-2 border-foreground/5 hover:border-foreground/10 hover:bg-foreground/5 ${
                        activeMetric === key
                  ...
                  <div className="flex h-auto min-w-[120px] items-center gap-x-2 rounded-medium bg-background p-2 text-tiny shadow-small">
                    <div className="flex w-full flex-col gap-y-0">
                      {payload?.map((p, index) => (
                        ...
                        >
                          <div className="flex w-full items-center gap-x-1 text-small text-foreground/90">
                            <span>{p.value}</span>
                            <span>{p.name}</span>
                          </div>
                        </div>
                      ))}
                      <span className="text-small font-medium text-foreground/40">
                        {label}
                      </span>
                    </div>
                  </div>
